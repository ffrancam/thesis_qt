...
class ConcludeGameAction:
    def __init__(self):
        rospy.init_node('conclude_game_action_node')
        print("Conclude Game Action Node started!")
        ...
        self.qt = QTRobot()
        print("Conclude Game Action Interface ready!")
        rospy.spin()

    def action_callback(self, msg):
        if msg.name != 'conclude_game':
            return
        ...
        if success:
            ...
            rospy.sleep(0.5)  # Simulate time taken to apply effects
            self._update_predicate(
                KnowledgeUpdateServiceRequest.ADD_KNOWLEDGE,
                'not_busy', []
            )
            rospy.sleep(0.5)  # Simulate time taken to update KB
    ...
    def apply_effects(self):
        """Applica gli effetti dell'azione conclude_game alla Knowledge Base"""
    ...
    def execute(self):
        try:
            ...
            #     (0, lambda: self.qt.talkText("Adesso giocheremo ad un gioco insieme!"))
            # ])
            self.qt.talkText("Grazie per aver giocato con me!")
            return True
        except rospy.ServiceException as e:
