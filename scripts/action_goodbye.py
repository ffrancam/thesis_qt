...
class GoodbyeAction:
    def __init__(self):
        rospy.init_node('goodbye_action_node')
        print("Goodbye Action Node started!")
        ...
        self.qt = QTRobot()
        print("Goodbye Action Interface ready!")
        rospy.spin()

    def action_callback(self, msg):
        if msg.name != 'say_goodbye':
            return
    ...
    def execute(self):
        try:
            results = self.qt.ts.sync([
                ...
                (0, lambda: self.qt.gesturePlay('QT/bye', 0)),
                (0, lambda: self.qt.talkText("Ciao, alla prossima!"))
            ])
            return True
        except rospy.ServiceException as e:
